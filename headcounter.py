    ...
    def run(self, stoplock, getlock, imglock, path, vertexes, threshold, history, confidence):
        ...
        while True:
            ...
            for x in self.track.objects:
                frame = cv.circle(frame, x[0], radius=3, color=(
                    0, 255, 0), thickness=-1)
            ...
            for x in range(0, len(vertexes)):
                frame = cv.line(
                    frame, vertexes[x], vertexes[x-1], (255, 0, 0), 4)

            for x in self.track.objects:
                if x[3]:
                    frame = cv.putText(
                        frame, "In", x[0], cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 6)
                else:
                    frame = cv.putText(
                        frame, "Out", x[0], cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 6)
            with imglock:
